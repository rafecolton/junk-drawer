<?
	...
	$description = nl2br(stripslashes($result["description"]));
	...
	echo("<p> <b>Date Assigned:</b> ".date("l, F j, Y \a\\t g:ia",strtotime($date_assigned))."</p>");
	echo("<p> <b>Description:</b> ".$description."</p>");
	echo("<p> <b>Due Date: </b> ".date("l, F j, Y \a\\t g:ia",strtotime($due_date))."</p>");
	echo("<p> <b>Late Due Date: </b> ".date("l, F j, Y \a\\t g:ia",strtotime($late_due_date))."</p>");
	echo("<p> <b>Number of Files Required:</b> ".$num_files."</p>");
	if ($is_open)
	{
...
function print_current_files()
{
	...
	else
	{
		...
		{
			if (!((strcasecmp($file, "late.txt") == 0) || (strcasecmp($file, "results.txt") == 0) || (substr_compare($file, ".class", -strlen(".class"), strlen(".class"), true) == 0)))
			{
